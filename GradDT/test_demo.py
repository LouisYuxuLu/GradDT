	...
	if IsGPU == 1:
		for f in range(len(testfiles)):
			...
			with torch.no_grad():
				...
				s = time.time()
				_,_,_,E_out = model(input_var)
				e = time.time()   
				print(input_var.shape)       
				print('GPUTime:%.4f'%(e-s))    
				E_out = chw_to_hwc(E_out.squeeze().cpu().detach().numpy())			               
				cv2.imwrite(result_dir + '/' + testfiles[f][:-4] + '_MTRBNet.png',np.clip(E_out*255,0.0,255.0))
		...
		for f in range(len(testfiles)):
			...
			with torch.no_grad():
				...
				s = time.time()
				_,_,_,E_out = model(input_var).to('cpu')
				e = time.time()   
				print(input_var.shape)       
				print('CPUTime:%.4f'%(e-s))    
				E_out = chw_to_hwc(E_out.squeeze().cpu().detach().numpy())			               
    
				cv2.imwrite(result_dir + '/' + testfiles[f][:-4] + '_GradTD.png',np.clip(E_out*255,0.0,255.0))
