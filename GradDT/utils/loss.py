...
def ms_ssim_L1(X, Y):
	return 1-ms_ssim(X,Y)# + 0.16*torch.mean(torch.cuda.FloatTensor(X.size()).normal_(mean =0,std =1.5)*torch.abs(X-Y))
...
class fixed_loss(nn.Module):
    ...
    def forward(self, out_image, gt_image):
        ...
        #loss = torch.mean(torch.pow((out_image - gt_image), 2)) + \
                #if_asym * 0.5 * torch.mean(torch.mul(torch.abs(0.3 - F.relu(gt_noise - est_noise)), ms_ssim_L1(est_noise , gt_noise))) + \
                #0.05 * tvloss   
        #loss = torch.mean(torch.pow((out_image - gt_image), 2)) + \
        ...
        loss = ms_ssim_L1(out_image,gt_image) # torch.mean(torch.pow(out_image - gt_image, 2))
        return loss
